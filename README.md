...
- **Subscriptions** — every detected recurring bill in one place: the full list (cadence, next
  charge), **upcoming charges** for the next 60 days, and **spending groups** (a donut + list of
  what you spend by category — Housing, Utilities, Fitness, etc).
- **Reports** — a 3/6/12-month view of income vs spending (bar chart), **net worth over time**
  (line chart), savings rate month by month, **category trends** (each category's shape over
  time, latest month scored against its own average), **top merchants**, and an **assets vs
  liabilities** breakdown.
- **Transactions** — **add one manually** (account, type — expense/income/transfer in or out —
  description, category, date, amount) with **advanced options** (tags, notes, link to a goal,
  exclude from budget); **rich filters** (search, income/expense/transfer, category, tag, account,
  account type, date range); free-form **#tags** with a **tags-in-use** bar and one-tap reuse;
  one-tap (sub)categorization; and inline **edit** of every field, including category, goal link,
  and the exclude-from-budget toggle. Auto-categorized on import.
...
- **Budgets** — envelope budgeting where you **deliberately allocate** each category (no wall of
  every category by default) — pick one from **Add allocation** and set a monthly limit; remove
  an allocation any time. Categories with spending but no allocation yet show in a compact
  **"spending without a budget"** list so nothing is hidden, with the same one-click allocate.
  **Categories & collapsible subcategories** (e.g. Transportation → Gas, Oil Change, Tires — click
  to expand), inline-editable limits, parent roll-ups, live progress; subcategory limits **can't
  exceed their parent** (clear error), and an **Uncategorized** section surfaces spending that
  isn't in any category.
...
- **Goals** — create and **edit** savings goals (name, target, date, icon) and fund them one of two
  ways: **link a funding account** — pick from every bank/broker account, grouped by institution
  and labeled with its product type — so progress tracks that account's balance automatically, or
  leave it unlinked and **tag any transaction** to the goal as a contribution. Either way, no
  manual top-ups. FinBud reads how fast the goal is actually growing and **projects the finish
  date**, flagging whether you're on track for your target date.
...
- **A deploy that crash-loops** is usually a config problem the boot check caught; the
  reason is the first error line in the deploy logs. See
  [if it won't start](#if-it-wont-start) — on a hosted platform this reads as a generic
  error page in the browser, because the container never comes up.
